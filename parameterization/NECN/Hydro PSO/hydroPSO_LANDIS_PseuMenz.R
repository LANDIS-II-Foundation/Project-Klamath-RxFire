...
##Getting the Flux tower observations to compare the results.

#obs <-read.table(paste(model.drty,"UMBSyrSoil2.txt", sep=""), fill=TRUE, header=F, sep="\t", skip=1)
obs_bio <- c(140000, 170000)  # max biomass and lai based on Sillett et al, 2018
obs_lai <- c(14.5, 13)
...
model.FUN.args=list(                                                        ### List with the arguments to be passed to 'model.FUN'.
  ...
  param.files=paste(model.drty, "PSO.in/ParamFiles_PseuMenz.txt", sep=""),            ### Name of the file storing the name of the files that have to be modified for each parameter.
...
max_observed_bio <-read.csv(paste0(FIA_results_dir, "Tree_Rel_Max_Biomass_from_FIA.csv", sep=""), fill=TRUE, header=T)
head(max_observed_bio)

MaxBiomass_bs <- subset(max_observed_bio, max_observed_bio$Species=="PseuMenz")
MaxBiomass_bs$Dataset <- "FIA"
MaxBiomass_bs <- MaxBiomass_bs[,-c(1,4)]
colnames(MaxBiomass_bs) <- c("Year", "AGB", "Dataset")
agb$Dataset <- "Simulated"
agb <- agb[,c("Year","AGB","Dataset")]
...
ggsave("PseuMenz_biomass_1startcohort_35particles.png",
       plot = last_plot(),
       path = "PSO.out/",
       dpi = 300,
       limitsize = TRUE)
...
##------------------
max_observed_lai <-read.csv(paste0(FIA_results_dir, "PLOT_TREES_LEAFAREA.csv", sep=""), fill=TRUE, header=T)
head(max_observed_lai)
...
ggsave("Salix_LAI_1startcohort_40particles.png",
       plot = last_plot(),
       path = "PSO.out/",
       dpi = 300,
       limitsize = TRUE)
