...
# obs <- obs[1:2,]
obs_bio <- c(8892, 6750)  #Compare Growth over time results -- values based on Conrad et al. 1982
obs_lai <- c(6, 5) # maximum LAI as reported by Hughes et al. 1987
...
model.FUN.args=list(                                                        ### List with the arguments to be passed to 'model.FUN'.
  ...
  param.files=paste(model.drty, "PSO.in/ParamFiles.txt", sep=""),            ### Name of the file storing the name of the files that have to be modified for each parameter.
...
max_observed <-read.csv(paste0(FIA_results_dir, "final_max_biomass_lai_shrubs_trees_from_FIA.csv", sep=""), fill=TRUE, header=T)

# MaxBiomasses <- NULL
# i=6
# for (i in 3:15){
#   
#   Max_obs_subset<-subset(Max_observed_bio, Max_observed_bio[,i]>0)
#   MaxBiomass_observed <- Max_obs_subset[,c(i,8)]
#   max_dataset<-rep("FIA", times=nrow(MaxBiomass_observed))
#   spp_dataset<-rep(names(MaxBiomass_observed)[1], times=nrow(MaxBiomass_observed))
#   MaxBiomass<-cbind.data.frame(max_dataset, MaxBiomass_observed[,1], spp_dataset, MaxBiomass_observed[,2:3])
#   colnames(MaxBiomass)<-c("Dataset", "Max_Biomass", "Species", "StandAge", "PlotLAI")
#   MaxBiomasses <- rbind(MaxBiomass, MaxBiomasses)
# }


MaxBiomass_bs <- subset(max_observed, max_observed$species=="Ceanothu")
MaxBiomass_bs$Dataset <- "FIA"
MaxBiomass_bs <- MaxBiomass_bs[,-c(1,4)]
MaxBiomass_bs <- MaxBiomass_bs[,c("age","max_biomass","maxLAI","Dataset")]
...
ggsave("Ceanothu_biomass_1startcohort_35particles.png",
       plot = last_plot(),
       path = "PSO.out/",
       dpi = 300,
       limitsize = TRUE)
...
ggsave("Ceanothu_LAI_1startcohort_35particles.png",
       plot = last_plot(),
       path = "PSO.out/",
       dpi = 300,
       limitsize = TRUE)
...
#plot limits
limits <- data_long <- gather(agb, AGB, TreeLAI, GrowthLimitLAI, GrowthLimitSoilWater, GrowthLimitT, 
                              GrowthLimitN, GrowthLimitLAIcompetition, factor_key=TRUE)
