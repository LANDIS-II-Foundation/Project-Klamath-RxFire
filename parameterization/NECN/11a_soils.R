...
landscape_mask <- raster("FinalProducts/final_IC_map.tif")
plot(landscape_mask)
...
writeRaster(final_drainage, "FinalProducts/soils/final_drainage.tif", datatype='FLT8S', overwrite=TRUE)
...
statsgo <- statsgo_shp %>% 
  ...
  dplyr::select(MUKEY, Field109) %>%
  left_join(chorizon, by= c("Field109" = "Field170")) %>%
  dplyr::select(MUKEY, Field109, Field92, Field95) %>%
  group_by(MUKEY) %>% 
